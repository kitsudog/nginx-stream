...
class HTTPParser:
    ...
    def starts_with_http_response(self, http_response: str):
    ...
    def parse_response(self, data: bytes, request: Dict) -> Optional[Dict]:
        ...
        origin = origin.decode("utf8", "replace")
        header = origin.split("\r\n")
        first_line, headers = header[0], header[1:]

        def kv(src: str):
            k, _, v = src.partition(":")
            return k, v.lstrip()

        origin_headers = dict(map(kv, headers))
        headers = self.headers_to_lower(origin_headers)
        first_line = first_line.split(" ")
        _, _, protocol = first_line[0].partition("/")
...
class Filter_bq(ResponseFilter):
...
class Filter_hq(ResponseFilter):
...
class Filter_tq(ResponseFilter):
...
def main():
    ...
    filter_expr_list = list(map(lambda x: eval(f"Filter_{x.group('mode')}({x.groupdict()})"), re.compile(
        ...
        r"( *(?P<regex>[^~]+[^~ ]))?"
    ).finditer(os.environ.get("FILTER_EXPR"))))
        ...
        for each in sorted(filter(lambda x: x.endswith(".pcap"), os.listdir(pcap_dir)))[:-1]:
            ...
            try:
                ...
                for packet in parser.read_pcap({"input": file}):
                    ...
                    if mongo:
                        mongo.update_one({"_id": f"{each}-{packet.no}"}, {"$set": record}, upsert=True)
            except Exception:
                traceback.print_exc()
            finally:
                try:
